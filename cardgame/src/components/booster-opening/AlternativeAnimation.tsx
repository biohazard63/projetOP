...
export default function UltraRareAnimation({ card, onComplete, shouldPlaySound = false, performanceMode = false }: Readonly<UltraRareAnimationProps>) {
  ...
  const { playAltArtSound, shouldPlaySound: canPlaySound } = useAudio()
  ...
  }, [onComplete, playAltArtSound, shouldPlaySound])
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          onClick={skip}
        >
          {/* Effet de fond (statique, theme alt) */}
          <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/15 via-green-500/15 to-teal-500/15" />
          ...
          {particleIds.map((id, i) => (
            ...
            >
              <Star className={`${isMobile ? 'w-3 h-3' : 'w-4 h-4'} text-emerald-400`} />
            </motion.div>
          ))}
          ...
          >
            ...
            >
              <div className="relative">
                ...
                <motion.div
                  className="absolute -inset-1 bg-gradient-to-r from-emerald-400 to-green-500 rounded-full blur-lg"
                  animate={{ opacity: [0.5, 0.8, 0.5], scale: [1, 1.1, 1] }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                />
                {/* Texte principal */}
                <motion.div
                  className="relative bg-gradient-to-r from-emerald-400 to-green-500 text-white px-6 py-3 rounded-full font-bold shadow-lg"
                  animate={{
                    boxShadow: [
            ...
            <div className="relative">
              ...
              <motion.div
                className={`absolute ${isMobile ? '-inset-2' : '-inset-4'} bg-gradient-to-r from-emerald-400 to-green-500 rounded-xl blur-xl`}
                animate={{ opacity: [0.5, 0.8, 0.5], scale: [1, 1.1, 1] }}
                transition={{ duration: 1.5, repeat: Infinity, ease: 'easeInOut' }}
              />
              {/* Effet de halo */}
              <motion.div
                className={`absolute ${isMobile ? '-inset-4' : '-inset-8'} bg-gradient-to-r from-emerald-400/30 to-green-500/30 rounded-xl blur-2xl`}
              ...
              {showPulse && (
                <>
                  <motion.div
                    ...
                    style={{ boxShadow: '0 0 0 2px rgba(52,211,153,.45), 0 0 60px rgba(34,197,94,.35) inset' }}
                  />
                  <motion.div
                    initial={{ opacity: 0 }}
              ...
              >
                <Sparkles className={`${isMobile ? 'w-8 h-8' : 'w-12 h-12'} text-emerald-400`} />
              </motion.div>
              ...
              >
                ...
                <p className={`${isMobile ? 'text-sm' : 'text-lg'} text-emerald-400 mt-1`}>{card.type}</p>
              </motion.div>
            </div>
          </motion.div>
