...
// Définition des decks de démarrage
const starterDecks = [
  {
...
async function createStarterDecksOnce() {
  try {
    console.log('Vérification des decks de démarrage existants...')
    
    // Vérifier si des decks de démarrage existent déjà
    ...
    if (existingDecks.length > 0) {
      ...
      console.log('Les decks de démarrage ne seront pas recréés.')
      return
    }
    
    console.log('Aucun deck de démarrage trouvé. Création des decks de démarrage...')
    ...
    for (const deckData of starterDecks) {
      ...
      const deck = await prisma.deck.create({
        data: {
          ...
          userId: null // Deck de démarrage sans utilisateur
        }
      })
      ...
      console.log(`Deck créé avec succès: ${deck.name}`)
    }
    
    console.log('Tous les decks de démarrage ont été créés avec succès!')
  } catch (error) {
